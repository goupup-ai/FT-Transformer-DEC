"""Standalone FT-Transformer implementation extracted from `rtdl_revisiting_models.py`.

This module contains only the FT-Transformer model and its direct dependencies,
so it can be imported and used independently, e.g.:
...
class LinearEmbeddings(nn.Module):
    ...
    **Shape**
    ...
    def __init__(self, n_features: int, d_embedding: int) -> None:
        ...
        if d_embedding <= 0:
            raise ValueError(
                f"d_embedding must be positive, however: {d_embedding=}"
            )
...
class MultiheadAttention(nn.Module):
    """Multihead (Self-/Cross-)Attention with optional Linformer-style compression."""
...
class FTTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        ffn_dropout: float,
        # NOTE: In the paper, FT-Transformer uses the ReGLU activation.
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if x.ndim != 3:
            raise ValueError(
                "The input must have exactly three dimension,"
                f" however: {x.ndim=}"
            )
    ...
    def __init__(
        ...
        **backbone_kwargs,
        ...
        if n_cont_features < 0:
            raise ValueError(
                "n_cont_features must be non-negative,"
                f" however: {n_cont_features=}"
            )
